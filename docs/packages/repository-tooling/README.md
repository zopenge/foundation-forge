...
| 按显式路径、事实与依赖解析适用工程规则，并只读检查/交付规则来源 | [`@openge/forge-repository-policy`](repository-policy.md) |
...
`repository-files` 负责 Git 文件发现；`repository-context` 不读取文件；`repository-policy` 只依据调用方声明的路径/事实选择并只读交付规则，不做源码检索；`text-integrity` 的 Node 入口可组合 `repository-files`；`source-snapshot` 组合通用仓库发现、内容完整性与安全发布能力。它们都不拥有具体产品的仓库语义。
